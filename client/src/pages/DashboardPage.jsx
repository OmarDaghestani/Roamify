...
import { heroImageFallback } from "../lib/imageFallbacks.js";
...
export default function DashboardPage() {
  ...
  function handleHeroImageError() {
    const fallback = heroImageFallback();
    if (heroSrc === fallback) return;
    setHeroSrc(fallback);
  }
  ...
  const tripFilterCounts = useMemo(() => {
    const planning = trips.filter(
      (t) =>
        (t.status === "draft" ? "planning" : t.status || "planning") ===
        "planning",
    ).length;
  ...
  const latestTripId =
    sortedTrips.find((t) => t.status !== "archived")?._id ||
    sortedTrips[0]?._id;
  ...
  async function handleDeleteAllPlanningConfirm() {
    ...
    try {
      ...
      if (deletedIds.length > 0) {
        setTrips((prev) =>
          prev.filter((t) => !deletedIds.includes(String(t._id))),
        );
      }
      if (failed > 0) {
        setError(
  ...
  return (
    <div className="dashboard-page">
      <section
        className="dashboard-hero dashboard-hero--above-atmosphere"
        aria-label="Featured destination backdrop"
      >
        ...
        <div className="dashboard-hero-inner">
          ...
          <p className="dashboard-lede">
            Curate escapes, set your compass on budget, and let the planner
            weave weather, FX, and languages into every decision.
          </p>
          <div className="dashboard-hero-actions">
            <button
              type="button"
              className="btn btn-new-trip"
            ...
            >
              {bulkDeleting
                ? "Deleting…"
                : `Delete all planning (${planningTrips.length})`}
            </button>
          </div>
        </div>
      </section>
            ...
            <div className="dashboard-main">
              ...
              {!loading && trips.length === 0 ? (
                <div className="dashboard-empty" role="status">
                  ...
                  <p className="dashboard-muted dashboard-empty-copy">
                    Start a journey from the button below, then refine details
                    on the trip page.
                  </p>
                  <ol className="dashboard-steps">
                    <li>
                      Click <strong>Begin your first journey</strong> and choose
                      currency and budget in the dialog.
                    </li>
                    <li>
                      Add dates, origin, and constraints on the trip page.
                    </li>
                    <li>
                      Open <strong>Planner chat</strong> and ask for
                      destinations within budget. Optional: adjust the dashboard
                      theme in <Link to="/settings">Settings</Link>.
                    </li>
                  </ol>
                  <button
                    type="button"
                    className="btn btn-new-trip btn-new-trip--inline"
                    onClick={openNewJourneyModal}
                  >
                    <span className="btn-new-trip-glow" aria-hidden />
                    <span className="btn-new-trip-inner">
              ...
              {!loading && trips.length > 0 ? (
                <>
                  {trips.length >= 2 ? (
                    <div
                      className="dashboard-trip-filter"
                      role="tablist"
                    ...
                    >
                      ...
                      >
                        All
                        <span className="dashboard-trip-filter__count">
                          {tripFilterCounts.all}
                        </span>
                      </button>
                      <button
                        type="button"
                  ...
                  {filteredTrips.length === 0 ? (
                    <div className="dashboard-filter-empty" role="status">
                      ...
                      <button
                        type="button"
                        className="btn secondary dashboard-filter-empty__reset"
                        onClick={() => setTripListFilter("all")}
                      >
                        Show all journeys
                      </button>
                      ...
                      {filteredTrips.map((t, index) => (
                        <TripCard
                          ...
                          onTripUpdated={(updated) =>
                            setTrips((prev) =>
                              prev.map((x) =>
                                String(x._id) === String(updated._id)
                                  ? { ...x, ...updated }
                                  : x,
                              ),
                            )
      ...
      {latestTripId ? (
        <Link
          to={`/trips/${latestTripId}#planner-chat`}
          className="fab-planner"
        ...
        <button
          type="button"
          className="fab-planner fab-planner--solo"
          onClick={openNewJourneyModal}
          title="New journey"
        >
          <span className="fab-planner-icon" aria-hidden>
